...
function felica_auth_admin_panels() {
	...
	// global options page
	$hookname = add_options_page(__('FeliCa Auth options', 'felica_auth'), __('FeliCa Auth', 'felica_auth'), 'manage_options', 'felica_auth', 'felica_auth_options_page' );
		...
		<table class="widefat">
			...
			<?php
				...
				if (empty($secret_keys)) {
					echo '<tr><td colspan="2">'.__('No Verified Accounts.', 'felica_auth').'</td></tr>';
				} else {
					foreach ($secret_keys as $secret_key) {
						echo '
						<tr>
...
function felica_auth_options_page() {
	...
	<div class="wrap">
		<form method="post" action="options.php">
			...
			<h2><?php _e('FeliCa Auth Settings', 'felica_auth') ?></h2>
